...
def _get_or_create_passcode_user(session: Session) -> User:
    ...
    passcode_user = getattr(settings, "passcode_user", None)
    passcode_hash = getattr(settings, "passcode_hash", None)
    
    if not passcode_user or not passcode_hash:
        # Passcode functionality disabled
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Passcode authentication not configured"
        )
    
    user = session.exec(select(User).where(User.username == passcode_user)).first()
    if user is None:
        # Create the passcode user with the configured hash
        user = User(
