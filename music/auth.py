...
def _get_or_create_passcode_user(session: Session) -> User:
    ...
    pass_code_user = settings.passcode_user
    pass_code_hash = settings.passcode_hash

    if not pass_code_user or not pass_code_hash:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Passcode authentication not configured"
        )

    user = session.exec(select(User).where(User.username == pass_code_user)).first()
    if user is None:
        user = User(
            username=pass_code_user,
            password_hash=pass_code_hash
        )
        session.add(user)
        session.commit()
