...
from auth import get_current_user_optional, verify_password, create_access_token, get_password_hash
from config import settings
from schemas import TokenResponse, UserProfile, UserCreate
...
async def get_playlists(
    ...
    try:
        ...
        # Order by folder_path (for explicit numbered ordering) then by name
        stmt = stmt.order_by(Playlist.folder_path, Playlist.name)
...
async def login(form_data: OAuth2PasswordRequestForm = Depends(), session: Session = Depends(get_session)):
    """Login with username/password. Returns an access token."""
    import bcrypt as _bcrypt
    user = session.exec(select(User).where(User.username == form_data.username)).first()
    if not user or not _bcrypt.checkpw(form_data.password.encode(), user.password_hash.encode()):
        raise HTTPException(
...
async def register(data: UserCreate, session: Session = Depends(get_session)):
    ...
    password_hash = _bcrypt.hashpw(data.password.encode(), _bcrypt.gensalt(rounds=12)).decode()
    user = User(username=data.username, password_hash=password_hash)
    session.add(user)
    session.commit()
    session.refresh(user)
    return UserProfile(id=user.id, username=user.username, created_at=user.created_at, updated_at=user.updated_at)
