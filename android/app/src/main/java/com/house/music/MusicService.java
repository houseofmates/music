...
public class MusicService extends Service
    ...
    private static final String PREFS_NAME = "music_service_state";
    ...
    public static final String ACTION_NEXT    = "com.house.music.NEXT";
    public static final String ACTION_PREVIOUS = "com.house.music.PREVIOUS";
    ...
    private void saveState() {
        try {
            ...
            prefs.edit()
                ...
                .putString("album", currentAlbum)
                .putInt("durationMs", currentDurationMs)
                .putInt("positionMs", mediaPlayer != null ? getCurrentPosition() : 0)
                .putInt("state", currentState)
                .apply();
        } catch (Exception e) {
    ...
    private void restoreState() {
        try {
            ...
            currentUrl = url;
            currentTrackId = prefs.getString("trackId", "");
            currentTitle = prefs.getString("title", "");
            currentArtist = prefs.getString("artist", "");
            currentAlbum = prefs.getString("album", "");
            currentDurationMs = prefs.getInt("durationMs", 0);
        ...
        try {
            switch (action) {
                ...
                case ACTION_STOP:
                    ...
                    break;
                case ACTION_NEXT:
                    // Notify JS layer to handle next track
                    notifyState(STATE_COMPLETED, getDuration());
                    break;
                case ACTION_PREVIOUS:
                    // Notify JS layer to handle previous track
    ...
    public void onPrepared(MediaPlayer mp) {
        ...
        isPreparing = false;
        try {
            // Apply pending seek if one was requested before preparation completed
            if (pendingSeekPosition > 0) {
                int pos = Math.min(pendingSeekPosition, mp.getDuration());
                mp.seekTo(pos);
            ...
            updateNotification(true);
            updateMediaSessionState();
            notifyState(STATE_PLAYING, getCurrentPosition());
        } catch (IllegalStateException e) {
...
builder.addAction(new Notification.Action.Builder(prevIcon, "Previous",
                PendingIntent.getService(this, 1,
                        new Intent(ACTION_PREVIOUS, null, this, MusicService.class)
                                .setPackage(pkgName),
...
builder.addAction(new Notification.Action.Builder(nextIcon, "Next",
                PendingIntent.getService(this, 3,
                        new Intent(ACTION_NEXT, null, this, MusicService.class)
                                .setPackage(pkgName),
                        PendingIntent.FLAG_UPDATE_CURRENT | PendingIntent.FLAG_IMMUTABLE)).build());
    ...
    public void onTaskRemoved(Intent rootIntent)
    {
        ...
        // Do NOT stop playback when app is swiped from recents.
        // Music should continue playing in background.
        // The foreground service lifecycle is managed by the app via the JS layer.
    }
