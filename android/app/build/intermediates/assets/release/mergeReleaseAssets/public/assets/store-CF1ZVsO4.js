...
var isAndroidNative = typeof window !== "undefined" && window.Capacitor && typeof window.Capacitor.getPlatform === "function" && window.Capacitor.getPlatform() === "android";
var explicitApiUrl = "https://music.houseofmates.space/api"?.trim() || "";
var DEFAULT_PROD_API_URL = "https://music.houseofmates.space/api";
var isWebBrowser = !isFileProtocol && !isAndroidNative;
var getFallbackApiBases = () => {
	if (isFileProtocol || isAndroidNative) return [
		DEFAULT_PROD_API_URL,
		"http://192.168.4.233:8000/api",
		"http://192.168.4.233:8000/api"
	];
	if (isAndroidNative) return [
		"http://192.168.4.233:8000/api",
		DEFAULT_PROD_API_URL,
		"http://192.168.4.233:3006/api",
		"http://192.168.4.233:8000/api",
		"http://127.0.0.1:3006/api"
	];
	if (isWebBrowser) return ["/api", DEFAULT_PROD_API_URL];
	return [
		"/api",
		DEFAULT_PROD_API_URL,
		"http://192.168.4.233:8000/api",
...
var isSafeBrowserApiUrl = (url) => {
	...
	try {
		...
		if (parsed.hostname === "music-backend" || parsed.hostname === "localhost" || parsed.hostname === "127.0.0.1") return false;
		return true;
	} catch {
		return false;
	}
};
var fallbackApiBases = getFallbackApiBases();
var safeExplicitApiUrl = explicitApiUrl && isSafeBrowserApiUrl(explicitApiUrl) ? explicitApiUrl : null;
var apiBases = safeExplicitApiUrl ? [safeExplicitApiUrl, ...fallbackApiBases.filter((base) => base !== safeExplicitApiUrl)] : isFileProtocol ? [explicitApiUrl || DEFAULT_PROD_API_URL, ...fallbackApiBases.filter((b) => b !== (explicitApiUrl || DEFAULT_PROD_API_URL))].filter(Boolean) : [...fallbackApiBases, ...!fallbackApiBases.includes(explicitApiUrl) && explicitApiUrl ? [explicitApiUrl] : []];
var API_BASE_URL = apiBases[0];
...
var resolveMediaUrl = (value) => {
	...
	return `${baseUrl.toString().replace(/\/$/, "")}/${value.replace(/^\/+/, "")}`;
};
var authTokenKey = "music_app_token";
var getAuthToken = () => {
...
var clearAuthToken = () => setAuthToken(null);
...
var testConnection = async (baseUrl) => {
	try {
...
var ensureConnection = async () => {
	...
	for (const baseUrl of apiBases) if (await testConnection(baseUrl)) {
...
if (isFileProtocol || isAndroidNative) ensureConnection();
api.interceptors.response.use((response) => {
	...
	if ((networkError || status === 403 || status === 502 || status === 503 || status === 504) && retried < apiBases.length - 1) {
		...
		const nextBase = apiBases[Math.min((startIndex < 0 ? 0 : startIndex) + retried + 1, apiBases.length - 1)];
...
var getTracks = (params = {}) => api.get("/tracks", {
	params: {
		limit: 50,
		...params
	},
...
var getTrack = (id) => api.get(`/tracks/${id}`);
var searchTracks = (query, searchIn = [
	"title",
	"artist",
	"album"
]) => api.post("/tracks/search", {
	query,
	search_in: searchIn
});
var getTrackStreamUrl = (id) => `${api.defaults.baseURL}/tracks/${id}/stream`;
var downloadTrack = (id) => api.get(`/tracks/${id}/download`, { responseType: "blob" });
var updateTrack = (id, data) => api.put(`/tracks/${id}`, data);
var getTrackLyrics = (id) => api.get(`/tracks/${id}/lyrics`);
var saveTrackLyrics = (id, lyrics) => api.put(`/tracks/${id}/lyrics`, { lyrics });
var saveSyncedLyrics = (id, synced_lyrics) => api.put(`/tracks/${id}/synced-lyrics`, { synced_lyrics });
var searchLyrics = (query) => api.get("/lyrics/search", { params: { q: query } });
var getArtists = () => api.get("/artists");
var getAlbums = (artist = null) => api.get("/albums", { params: { artist } });
var mergeAlbums = (data) => api.post("/albums/merge", data);
var getPlaylists = () => api.get("/playlists");
var getPlaylist = (id) => api.get(`/playlists/${id}`);
var createPlaylist = (data) => api.post("/playlists", data);
var updatePlaylist = (id, data) => api.patch(`/playlists/${id}`, data);
var deletePlaylist = (id) => api.delete(`/playlists/${id}`);
var uploadTrackCover = (id, file) => {
	const form = new FormData();
	form.append("file", file);
	return api.post(`/tracks/${id}/cover`, form, { headers: { "Content-Type": "multipart/form-data" } });
};
var uploadAlbumCover = (album, artist, file) => {
	const form = new FormData();
...
var uploadArtistCover = (artist, file) => {
	const form = new FormData();
	form.append("artist", artist);
	form.append("file", file);
	return api.post("/artists/cover", form, { headers: { "Content-Type": "multipart/form-data" } });
};
var renameArtist = (oldName, newName) => {
	const form = new FormData();
	form.append("old_name", oldName);
	form.append("new_name", newName);
	return api.post("/artists/rename", form);
};
var addTrackToPlaylist = (playlistId, trackId, position = null) => api.post(`/playlists/${playlistId}/tracks`, {
	track_id: trackId,
	position
});
var removeTrackFromPlaylist = (playlistId, trackId) => api.delete(`/playlists/${playlistId}/tracks/${trackId}`);
var reorderPlaylistTracks = (playlistId, trackIds) => api.put(`/playlists/${playlistId}/tracks/reorder`, { track_ids: trackIds });
var getQueue = () => api.get("/queue");
var addToQueue = (trackId, position = null) => api.post("/queue", {
	track_id: trackId,
	position
});
var reorderQueue = (queueItemId, newPosition) => api.post("/queue/reorder", {
	queue_item_id: queueItemId,
	new_position: newPosition
});
var removeFromQueue = (queueItemId) => api.delete(`/queue/${queueItemId}`);
var clearQueue = () => api.delete("/queue");
var getPlayerState = () => api.get("/player/state");
var updatePlayerState = (data) => api.patch("/player/state", data);
var register = (username, password) => api.post("/auth/register", {
	username,
	password
});
var login = (username, password) => api.post("/auth/token", new URLSearchParams({
	username,
	password
}), { headers: { "Content-Type": "application/x-www-form-urlencoded" } });
var getMe = () => api.get("/auth/me");
var getSettings = () => api.get("/users/me/settings");
var updateSettings = (data) => api.patch("/users/me/settings", data);
var getFavorites = () => api.get("/users/me/favorites");
var addFavorite = (trackId) => api.post("/users/me/favorites", { track_id: trackId });
var removeFavorite = (trackId) => api.delete(`/users/me/favorites/${trackId}`);
var getHistory = (limit = 50) => api.get("/users/me/history", { params: { limit } });
var addHistory = (trackId, position, duration) => api.post("/users/me/history", {
	track_id: trackId,
	position,
	duration
});
var getShareLink = (token) => api.get(`/share/${token}`);
var searchMusicDl = (q) => api.get("/music-dl/search", { params: { q } });
var listMusicDlFolders = () => api.get("/music-dl/folders");
var createMusicDlFolder = (name) => api.post("/music-dl/folders", { name });
var downloadMusicDl = (data = {}) => api.post("/music-dl/download", data);
//#endregion
//#region src/store.js
var usePlayerStore = create((set, get) => ({
	...
	loadSettings: async () => {
		try {
			...
			if (response?.data) set({ settings: response.data });
		} catch (error) {
	...
	updateSettings: async (partialSettings = {}) => {
		...
		try {
			const nextSettings = (await updateSettings(payload))?.data || payload;
	...
	loadFavorites: async () => {
		try {
			...
			set({ favorites: Array.isArray(response?.data) ? response.data.map((item) => item.track) : [] });
		} catch (error) {
	...
	setIsPlaying: (playing) => {
		...
		updatePlayerState({ is_playing: playing });
	},
	setVolume: (volume) => {
		set({ volume });
		const { audioRef } = get();
	...
	setRepeatMode: (mode) => {
		set({ repeatMode: mode });
		updatePlayerState({ repeat_mode: mode });
	},
	toggleShuffle: () => {
		const { shuffle } = get();
		set({ shuffle: !shuffle });
		updatePlayerState({ shuffle: !shuffle });
	},
	toggleLyrics: () => {
	...
	preloadNextTrack: () => {
		const { queue, currentQueueIndex, settings, audioRef } = get();
		if (!settings.isCrossfadeEnabled || !audioRef || queue.length === 0) return null;
		let nextIndex = currentQueueIndex + 1;
		if (nextIndex >= queue.length) if (get().repeatMode === "all") nextIndex = 0;
		else return null;
		const nextTrack = queue[nextIndex]?.track;
		if (!nextTrack) return null;
		set({ nextTrackPreloaded: true });
		return {
			track: nextTrack,
			index: nextIndex
		};
	...
	startCrossfade: () => {
		const { audioRef, settings, currentQueueIndex, queue, repeatMode, shuffle, volume } = get();
		if (!settings.isCrossfadeEnabled || !audioRef) return null;
		let nextIndex;
		if (repeatMode === "one") nextIndex = currentQueueIndex;
		else if (shuffle) nextIndex = Math.floor(Math.random() * queue.length);
		else {
			nextIndex = currentQueueIndex + 1;
			if (nextIndex >= queue.length) if (repeatMode === "all") nextIndex = 0;
			else return null;
		}
		const nextTrack = queue[nextIndex]?.track;
		if (!nextTrack) return null;
		...
		crossfadeAudio.src = getTrackStreamUrl(nextTrack.id);
		crossfadeAudio.volume = 0;
		crossfadeAudio.load();
		const fadeDuration = (settings.crossfadeSeconds || .8) * 1e3;
		set({
			isCrossfading: true,
			crossfadeAudioRef: crossfadeAudio,
			nextTrackPreloaded: false
		});
		crossfadeAudio.play().catch(() => {});
		get().fadeAudio(crossfadeAudio, volume, fadeDuration);
		get().fadeAudio(audioRef, 0, fadeDuration);
		setTimeout(() => {
			audioRef.src;
			audioRef.src = "";
			audioRef.load();
			audioRef.src = crossfadeAudio.src;
			...
			if (crossfadeAudio.parentNode) crossfadeAudio.parentNode.removeChild(crossfadeAudio);
			set({
	...
	playTrack: async (track, queueIndex = -1) => {
		const { audioRef, settings, volume, audioContext } = get();
		if (audioRef) {
			audioRef.pause();
			try {
				audioRef.src = "";
		...
		if (audioRef) {
			audioRef.src = getTrackStreamUrl(track.id);
			audioRef.load();
			try {
				await audioRef.play();
			} catch (error) {
		...
		try {
			await addHistory(track.id, 0, track.duration);
		} catch (err) {
	...
	playPause: async () => {
		...
		if (isPlaying) {
			audioRef.pause();
			set({ isPlaying: false });
			updatePlayerState({ is_playing: false });
		} else try {
			await audioRef.play();
			set({ isPlaying: true });
			updatePlayerState({ is_playing: true });
		} catch (error) {
			console.error("Error playing:", error);
		}
	},
	nextTrack: () => {
		const { queue, currentQueueIndex, repeatMode, shuffle } = get();
		...
		const nextTrack = queue[nextIndex]?.track;
		if (nextTrack) get().playTrack(nextTrack, nextIndex);
	},
	previousTrack: () => {
		...
		if (prevTrack) get().playTrack(prevTrack, prevIndex);
	},
	seekTo: (position) => {
		const { audioRef, isPlaying } = get();
		if (audioRef) {
			audioRef.currentTime = position;
			if (isPlaying && audioRef.paused) audioRef.play().catch((error) => {
				console.error("Error resuming after seek:", error);
			});
			set({ currentPosition: position });
			updatePlayerState({ current_position: position });
		}
	},
	jump: (seconds) => {
	...
	loadQueue: async () => {
		try {
			...
			set({ queue: Array.isArray(response?.data) ? response.data : [] });
		} catch (error) {
	...
	reorderQueueItem: async (queueItemId, newPosition) => {
		try {
			await reorderQueue(queueItemId, newPosition);
			await get().loadQueue();
		} catch (error) {
	...
	loadPlayerState: async () => {
		try {
			const state = (await getPlayerState()).data;
			if (state.current_track) {
				...
				set({
					...
					isPlaying: false,
					volume: state.volume ?? .8,
					repeatMode: state.repeat_mode || "none",
					shuffle: state.shuffle || false
				});
				if (audioRef) {
					audioRef.src = getTrackStreamUrl(state.current_track.id);
					audioRef.load();
					audioRef.currentTime = state.current_position || 0;
					audioRef.volume = state.volume ?? .8;
				}
			}
		} catch (error) {
...
//#endregion
export { updatePlaylist as A, reorderPlaylistTracks as C, searchLyrics as D, saveTrackLyrics as E, require_jsx_runtime as F, require_react as I, __commonJSMin as L, uploadAlbumCover as M, uploadArtistCover as N, searchMusicDl as O, uploadTrackCover as P, __toESM as R, renameArtist as S, saveSyncedLyrics as T, getTrackStreamUrl as _, createPlaylist as a, mergeAlbums as b, downloadTrack as c, getHistory as d, getPlaylist as f, getTrackLyrics as g, getTrack as h, createMusicDlFolder as i, updateTrack as j, searchTracks as k, getAlbums as l, getShareLink as m, addTrackToPlaylist as n, deletePlaylist as o, getPlaylists as p, api as r, downloadMusicDl as s, usePlayerStore as t, getArtists as u, getTracks as v, resolveMediaUrl as w, removeTrackFromPlaylist as x, listMusicDlFolders as y };
