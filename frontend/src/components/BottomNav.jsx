...
export default function BottomNav({ onRevealPlayer, isPlayerHidden = false }) {
  useEffect(() => {
    console.log('[BottomNav] MOUNTED build=' + BUILD_ID);
...
{/* ===== DESKTOP ONLY: Player bar ABOVE the nav bar ===== */}
 ...
 <input
  ...
  onMouseDown={() => { desktopInteractingRef.current = true; }}
  onMouseUp={() => { desktopInteractingRef.current = false; }}
  onTouchStart={() => { desktopInteractingRef.current = true; }}
  onTouchEnd={() => { desktopInteractingRef.current = false; }}
  onInput={(e) => {
...
{/* ===== MOBILE PLAYER STRIP (hidden on desktop) ===== */}
  <div className={"md:hidden flex-shrink-0 mobile-player-strip bg-[#050505] " + (showPlayerControls ? "" : "hidden")}>
...
{/* Controls + progress live in a flex column. Flex `gap` never collapses,
  ...
  <div className="flex flex-col gap-0 px-2 py-0">
   ...
   <div className="flex items-center justify-center gap-1 flex-shrink-0">
     <button
     onClick={previousTrack}
     className="w-10 h-10 flex items-center justify-center text-white rounded-full active:scale-90 transition-none"
     aria-label="previous"
     >
...
<button
       ...
       className="w-10 h-10 flex-shrink-0 flex items-center justify-center bg-[#ffb10f] text-black rounded-full active:scale-90 transition-none"
       aria-label={isPlaying ? 'pause' : 'play'}
       >
      {isPlaying
   ...
   {navItems.map((item) => {
     ...
     return (
       <button
         ...
         className={`flex flex-col items-center justify-center flex-1 h-full transition-none ${isActive ? 'text-[#f6b012]' : 'text-white'} md:hover:text-[#ffd86a]`}
       >
