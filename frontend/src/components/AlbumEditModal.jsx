...
export default function AlbumEditModal({ isOpen, albumData, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      // pass file separately so caller can upload it
      await onSave({ album, artist, cover_art_url: cover, coverFile });
      onClose();
    } catch (err) {
    } finally {
      setSaving(false);
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-2xl font-bold text-vibe-gold mb-4 lowercase">edit album</h2>
